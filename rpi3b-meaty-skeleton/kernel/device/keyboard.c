...
void KbdLoad()
{
    printk("CSUD: Keyboard driver version 0.1\n");
    keyboardCount = 0;
...
uint8_t KeyboardGetChar(uint32_t address)
{
    for (uint32_t i = 0; i < 6; i++)
    {
        ...
        // printk("KeyboardGetKeyDown: %x", scanCode);
        if (scanCode == 0)
        {
            return 0;
        }
...
Result KeyboardAttach(struct UsbDevice *device, __attribute__((__unused__)) uint32_t interface)
{
    ...
    {
        printk("KBD: Warning! KeyboardMaxKeyboards not a multiple of 4. The driver wasn't built for this!\n");
    }
    if (keyboardCount == KeyboardMaxKeyboards)
    {
        printk("KBD: %s not connected. Too many keyboards connected (%d/%d). Change KeyboardMaxKeyboards in device.keyboard.c to allow more.\n", UsbGetDescription(device), keyboardCount, KeyboardMaxKeyboards);
    ...
    {
        printk("KBD: %s isn't a HID device. The keyboard driver is built upon the HID driver.\n", UsbGetDescription(device));
        return ErrorIncompatible;
    }
    ...
    {
        printk("KBD: Not enough memory to allocate keyboard %s.\n", UsbGetDescription(device));
        return ErrorMemory;
    }
    ...
    {
        printk("KBD: PANIC! Driver in inconsistent state! KeyboardCount is inaccurate.\n");
    ...
    *(uint8_t *)&data->LedSupport = 0;
    ...
    for (uint32_t i = 0; i < parse->ReportCount; i++)
    {
                 ...
                 data->LedReport == NULL)
        {
            ...
            printk("KBD: Input report %d. %d fields.\n", i, parse->Report[i]->FieldCount);
            for (uint32_t j = 0; j < parse->Report[i]->FieldCount; j++)
            {
    ...
    printk("KBD: New keyboard assigned %d!\n", device->Number);
...
Result KeyboardPoll(uint32_t keyboardAddress)
{
    ...
    if ((result = HidReadDevice(keyboards[keyboardNumber], data->KeyReport->Index)) != OK)
    {
        if (result != ErrorDisconnected)
            printk("KBD: Could not get key report from %s.\n", UsbGetDescription(keyboards[keyboardNumber]));
        return result;
    }
