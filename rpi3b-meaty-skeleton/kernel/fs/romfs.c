...
retry_inode:
	...
	printk("romfs: Attempting to stat inode %x\n",inode);
	...
	switch(type) {
		...
		case 4: /* block device */
			...
			break;
		case 5: /* uint8_t device */
			printk("uint8_t DEVICE\n");
			buf->st_mode|=S_IFCHR;
			buf->st_rdev=spec_info;
			break;
		case 6: /* socket */
...
static uint32_t romfs_get_size(struct superblock_t *superblock) {
    ...
    printk("superblock: %x", superblock);
	/* Read header */
...
int32_t romfs_read_file(uint32_t inode,
	...
	printk("romfs: Attempting to read %d bytes from inode %x offset %d\n",
	...
	printk("romfs: inode %d name_length %d header_offset %d\n",
			inode,name_length,header_offset);

	/* Return data */
	/* FIXME: copy_to_user() */

	printk("romfs: max count %d, size is %d\n",count,size);
	max_count=count;
	if (max_count>size-file_offset) {
		max_count=size-file_offset;
		printk("romfs: count is past end of file, limiting to %d\n",max_count);
	}

	printk("romfs: reading %d bytes from %d into %x\n",
		max_count,header_offset+file_offset,buf);

	read_count=ramdisk_read(header_offset+file_offset,max_count,buf);

	printk("romfs: result was %d\n",read_count);
...
int32_t romfs_getdents(uint32_t dir_inode,
	...
	while(1) {
		...
		dirent_ptr=(struct vmwos_dirent *)(((uint8_t *)dirent_ptr)+current_length);
	}
